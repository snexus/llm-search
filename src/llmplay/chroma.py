...
class ChromaVS:
    def __init__(
        self, persist_folder: str,  collection_name: str, embedding_model_name: str, service_context, index_id: str = "vector_index"
    ):
    ...
    def create_index_from_folder(self, folder_path: str, parser_func: Callable, extension="md"):    
        ...
        index = GPTVectorStoreIndex(nodes=nodes, storage_context=storage_context, service_context=self._service_context)                          
    ...
    def load_index(self):
        ...
        index = load_index_from_storage(storage_context=sc, index_id=self._index_id, service_context = self._service_context)
        return index
