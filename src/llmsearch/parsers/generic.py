    ...
    def split_document(self, document_path: Union[str, Path], max_size: int) -> List[dict]:
        ...
        for el in elements:
            ...
            if len(text) >= max_size-1:
                ...
                all_chunks.append({'text': current_chunk, 'metadata': {'page': current_page}})
                
                texts = text_split(text, max_size)
                logger.info(f"Sub-splitting large chunk. Got {len(texts)} sub-chunks.")
                for t in texts:
                     all_chunks.append({'text': "\n"+t, 'metadata': {'page': el.metadata.page_number}})
            ...
            # if element's text doesn't fit into current chunk - flush the current chunk and create a new chunk
                ...
                all_chunks.append({'text': current_chunk, 'metadata': {'page': current_page}})
                logger.info(f"Flushing chunk. Length: {len(current_chunk)}, page: {el.metadata.page_number}")
                current_chunk = text
