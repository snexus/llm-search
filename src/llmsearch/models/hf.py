...
class HuggingFaceModel(AbstractLLMModel):
    ...
    def model(self):
        tokenizer_name = (
            self.config.tokenizer_name if self.config.tokenizer_name is not None else self.config.model_name
        )
        tokenizer = AutoTokenizer.from_pretrained(tokenizer_name, **self.config.tokenzier_kwargs)
