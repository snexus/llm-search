...
class SparseEmbeddingsSplade:
    ...
    def query(self, search: str, chunk_size: int, n: int = 50, label: str = "") -> Tuple[List[str], np.array]:
        ...
        if label and label in self._labels_to_ind and self._embeddings is not None and self._ids is not None:
            ...
            logger.info(f"SPLADE - restricting search to label: {label}, chunk size: {chunk_size}. Number of docs: {len(indices)}")
            
        
        else:
            indices = sorted(list(set(self._chunk_size_to_ind[chunk_size])))
            logger.info(f"SPLADE search will search over all documents of chunk size: {chunk_size}. Number of docs: {len(indices)}")
        ...
        if (
            ...
            and l2_norm_matrix is not None
            and ids is not None
        ):
