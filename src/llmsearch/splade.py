...
if __name__ == "__main__":
    ...
    config = get_config("notebooks/dev/dev_config.yaml")
    
    # splitter = DocumentSplitter(config)
    # docs = splitter.split()
    
    splade = SparseEmbeddingsSplade(config=config)
    #splade.generate_embeddings_from_docs(docs)
    ids = splade.query(search="how to overwrite a table in Spark?", n = 10)
    print(ids)
